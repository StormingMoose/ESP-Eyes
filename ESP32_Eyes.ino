...
#define JOYTIMEOUT        1030      // joystick no activity timeout in mS

#define JoyStickDeadZone  100
...
#define GREENLED    13      // GPIO 15 -> Green LED - lit -> scanning
#define BLUELED     25       // GPIO 2  -> Blue LED  - lit -> connected
#define REDLED      14       // GPIO 4  -> Red LED
...
void taskJoyStick(void *parameter)
{
  ...
  while(true)
  {
    ...
    if (y < JoyStickDeadZone-100)
    {
      // move forward
      Serial.println("Forward");
      
      //===== add your code here =====
       drawBmp("/blue_eye_u.bmp", 0, 0);
       delay(100);
      
    }
    else if (y > JoyStickDeadZone-100)
    {
      // move backward
      Serial.println("Backward");

      //===== add your code here =====
       drawBmp("/blue_eye_d.bmp", 0, 0);
       delay(100);
    }
        
    if (x > JoyStickDeadZone-100)
    {
      // turn right
      Serial.println("Turn Right");

      //===== add your code here =====
       drawBmp("/blue_eye_r.bmp", 0, 0);
       delay(100);
    }
    else if (x < JoyStickDeadZone-100)
    {
      // turn left
      Serial.println("Turn Left");

      //===== add your code here =====
       drawBmp("/blue_eye_l.bmp", 0, 0);
       delay(100);
    }
...
//  *********************************************************************************************  
  ...
  for(int i = 0; i < NSTARS; ++i)
  {
    ...
    else
    {
      ...
      // This is a faster pixel drawing function for occassions where many single pixels must be drawn
      tft.drawPixel(old_screen_x, old_screen_y,TFT_RED);
